package com.mes.exc.server.shristool;
...
import com.mes.exc.server.utils.DBHelper;
...
public class LoggerTool {
	...
	private static NamedParameterJdbcTemplate nameJdbcTemplate = null;
	...
	public static boolean SaveException(String wModuleName, String wFunctionName, String wBugInfo) {
		try {
			if (!LoggerTool.Init)
				return false;
			...
			if (wSessionTime.LoggerEnable) {
				...
				// ==============创建保存文件==============
				WriteFile(wPath_File, wMessageText);
			}
			return true;
		} catch (Exception ex) {
			return false;
	...
	public static boolean SaveLogger(String wModuleName, String wFunctionName, String wBugInfo) {
		try {
			if (!LoggerTool.Init)
				return false;
	...
	private static synchronized void WriteFile(String wPath_File, String wMessage) {

		OutputStreamWriter os = null;
		BufferedWriter bw = null;
		PrintWriter out = null;
		try {
			os = new OutputStreamWriter(new FileOutputStream(wPath_File, true), "UTF-8");
			bw = new BufferedWriter(os);
			out = new PrintWriter(bw);
			out.println(wMessage);
			out.flush();
