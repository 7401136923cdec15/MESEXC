...
//						WDWConstans.SetBizTask(wBPMTaskBase);
	...
	@Override
	public APIResult BPM_DelegateTask(BMSEmployee wLoginUser, String taskId, String userId, String wRemark) {
		APIResult wResult = new APIResult();
		try {
			Map<String, Object> wParms = new HashMap<String, Object>();
			wParms.put("taskId", taskId);
			wParms.put("userId", userId);
			wParms.put("remark", wRemark);
			wResult = RemoteInvokeUtils.getInstance().HttpInvokeAPI(ServerUrl, ServerName,
					StringUtils.Format("api/Runtime/delegateTask?cadv_ao={0}&cade_po={1}", wLoginUser.getLoginName(),
							wLoginUser.getPassword()),
					wParms, HttpMethod.POST);
		} catch (Exception ex) {
			logger.error(ex.toString());
